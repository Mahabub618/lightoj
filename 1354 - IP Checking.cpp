    ...
    int32_t main()
    {
        ...
        FastIO;
        long long int a, b, c, d, a1, b1, c1, d1, k, l, m, x=1, n, t;
        char p, q, r, p1, q1, r1;
        scanf("%lld", &t);
        while(t--)
        {
            scanf("%lld %c %lld %c %lld %c %lld", &a, &p, &b, &q, &c, &r, &d);
            scanf("%lld %c %lld %c %lld %c %lld", &a1, &p1, &b1, &q1, &c1, &r1, &d1);
            k = convertBinaryToDecimal(a1);
            l = convertBinaryToDecimal(b1);
            m = convertBinaryToDecimal(c1);
            n = convertBinaryToDecimal(d1);
            if(a+b+c+d == k+l+m+n) printf("Case %lld: Yes\n", x++);
