  ...
  /*****************************************************************\
    ...
    int row, col, cnt;
    bool vis[21][21];
    int isvalid(int row1, int col1)
    {
        if(row1>=0 and row1<row and col1>=0 and col1<col and arr[row1][col1]=='.' and !vis[row1][col1]) return 1;
        else return 0;
    }
    void dfs(int x, int y)
    {
        ...
        for(int k=0; k<4; k++)
        {
            ...
            int res = isvalid(row1, col1);
            if(res)
            {
                cnt++;
                //cout << "row = " << row1 << " col = " << col1 << endl;
                dfs(row1, col1);
            }
        }
    }
